...
UIFactory is a low-code internal-app platform. Users sign in with SSO, add connectors, build multi-page apps by drag-and-drop, AI generation, or hand-written HTML plus chat pages, then version, deploy, and share them. The repo is an npm workspaces monorepo with a NestJS API in `backend/`, a React 19 + Vite SPA in `frontend/`, docs, and an AKS Helm chart.
...
|-- CLAUDE.md            # compatibility pointer to this AGENTS hierarchy
...
- There is no lint step; verify with backend build/tests and frontend typecheck/build as applicable.
